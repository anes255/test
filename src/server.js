...
// Platform info
app.get('/api/platform-info',async(req,res)=>{try{const r=await pool.query('SELECT * FROM platform_settings LIMIT 1');const s=r.rows[0]||{};res.json({site_name:s.site_name||'KyoMarket',site_logo:s.logo_url,primary_color:s.primary_color||'#7C3AED',secondary_color:s.secondary_color||'#06B6D4',accent_color:s.accent_color||'#F59E0B',meta_description:s.meta_description,favicon:s.favicon_url,maintenance_mode:s.maintenance_mode,currency:s.currency||'DZD',landing_blocks:s.landing_blocks||'[]',google_client_id:s.google_client_id||''});}catch(e){res.json({site_name:'KyoMarket',landing_blocks:'[]'});}});
