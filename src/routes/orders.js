...
// Delivery companies
router.get('/stores/:sid/delivery-companies',authMiddleware(['store_owner']),async(req,res)=>{try{res.json((await pool.query('SELECT * FROM delivery_companies WHERE store_id=$1',[req.params.sid])).rows);}catch(e){res.json([]);}});
router.post('/stores/:sid/delivery-companies',authMiddleware(['store_owner']),async(req,res)=>{try{const{name,api_key,base_rate}=req.body;const r=await pool.query('INSERT INTO delivery_companies(store_id,name,api_key,base_rate) VALUES($1,$2,$3,$4) RETURNING *',[req.params.sid,name,api_key||null,base_rate||0]);res.status(201).json(r.rows[0]);}catch(e){res.status(500).json({error:e.message});}});
