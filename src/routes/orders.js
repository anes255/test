...
router.put('/stores/:sid/shipping-wilayas',authMiddleware(['store_owner']),async(req,res)=>{const client=await pool.connect();try{await ensureShippingCols();const sid=req.params.sid;const{wilayas,shipping_mode,free_shipping_enabled,free_shipping_threshold}=req.body||{};await client.query('BEGIN');await client.query('UPDATE stores SET shipping_mode=COALESCE($2,shipping_mode),free_shipping_enabled=COALESCE($3,free_shipping_enabled),free_shipping_threshold=COALESCE($4,free_shipping_threshold) WHERE id=$1',[sid,shipping_mode||null,typeof free_shipping_enabled==='boolean'?free_shipping_enabled:null,typeof free_shipping_threshold==='number'?free_shipping_threshold:null]);if(Array.isArray(wilayas)){for(const w of wilayas){const active=w.is_active!==false;if(w.id){await client.query('UPDATE shipping_wilayas SET wilaya_name=$2,wilaya_code=$3,desk_delivery_price=$4,home_delivery_price=$5,delivery_days=$6,is_active=$7 WHERE id=$1 AND store_id=$8',[w.id,w.wilaya_name,w.wilaya_code,w.desk_delivery_price||0,w.home_delivery_price||0,w.delivery_days||3,active,sid]);}else{await client.query('INSERT INTO shipping_wilayas(store_id,wilaya_name,wilaya_code,desk_delivery_price,home_delivery_price,delivery_days,is_active) VALUES($1,$2,$3,$4,$5,$6,$7) ON CONFLICT DO NOTHING',[sid,w.wilaya_name,w.wilaya_code,w.desk_delivery_price||0,w.home_delivery_price||0,w.delivery_days||3,active]);}}}await client.query('COMMIT');res.json({ok:true});}catch(e){await client.query('ROLLBACK');res.status(500).json({error:e.message});}finally{client.release();}});
router.put('/stores/:sid/shipping-wilayas/:wid',authMiddleware(['store_owner']),async(req,res)=>{try{await ensureShippingCols();const{wilaya_name,wilaya_code,desk_delivery_price,home_delivery_price,delivery_days,is_active}=req.body;const r=await pool.query('UPDATE shipping_wilayas SET wilaya_name=COALESCE($3,wilaya_name),wilaya_code=COALESCE($4,wilaya_code),desk_delivery_price=COALESCE($5,desk_delivery_price),home_delivery_price=COALESCE($6,home_delivery_price),delivery_days=COALESCE($7,delivery_days),is_active=COALESCE($8,is_active) WHERE id=$1 AND store_id=$2 RETURNING *',[req.params.wid,req.params.sid,wilaya_name||null,wilaya_code||null,desk_delivery_price??null,home_delivery_price??null,delivery_days??null,typeof is_active==='boolean'?is_active:null]);res.json(r.rows[0]||{ok:true});}catch(e){res.status(500).json({error:e.message});}});
