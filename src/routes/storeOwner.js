...
// Staff
router.get('/stores/:sid/staff',authMiddleware(['store_owner']),async(req,res)=>{try{res.json((await pool.query('SELECT id,name,email,phone,role,permissions,role_template_id,is_active,created_at FROM store_staff WHERE store_id=$1',[req.params.sid])).rows);}catch(e){res.json([]);}});
router.post('/stores/:sid/staff',authMiddleware(['store_owner']),_lpf,_eq({type:'staff'}),async(req,res)=>{try{const{name,email,phone,password,role}=req.body;let{permissions}=req.body;let roleTemplateId=null;if(typeof role==='string'&&role.startsWith('tpl_')){roleTemplateId=role.slice(4);try{const tpl=await pool.query('SELECT permissions FROM role_templates WHERE id=$1',[roleTemplateId]);if(tpl.rows.length&&!permissions)permissions=tpl.rows[0].permissions;}catch(e){}}if(permissions&&typeof permissions!=='string')permissions=JSON.stringify(permissions);const hash=await bcrypt.hash(password,12);const r=await pool.query('INSERT INTO store_staff(store_id,name,email,phone,password_hash,role,permissions,role_template_id) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id,name,email,phone,role,permissions,role_template_id,created_at',[req.params.sid,name,email||null,phone||null,hash,role||'viewer',permissions||'[]',roleTemplateId]);res.status(201).json(r.rows[0]);}catch(e){res.status(500).json({error:e.message});}});
